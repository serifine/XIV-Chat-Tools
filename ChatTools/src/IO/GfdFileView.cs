...
namespace ChatTools.IO;

// Credit to https://github.com/Soreepeong/Dalamud/blob/feature/log-wordwrap/Dalamud/Interface/Spannables/Internal/GfdFileView.cs
// Changes made to remove unsafe keyword.

/// <summary>Reference member view of a .gfd file data.</summary>
// internal readonly unsafe ref struct GfdFileView // original: unsafe required for fixed byte fields
internal readonly ref struct GfdFileView
{
    ...
    {
        this.span = span;
        if (span.Length < Unsafe.SizeOf<GfdHeader>()) // original: sizeof(GfdHeader)
            throw new InvalidDataException($"Not enough space for a {nameof(GfdHeader)}");
        if (span.Length < Unsafe.SizeOf<GfdHeader>() + (this.Header.Count * Unsafe.SizeOf<GfdEntry>())) // original: sizeof(GfdHeader) + (... * sizeof(GfdEntry))
    ...
    public ReadOnlySpan<GfdEntry> Entries => MemoryMarshal.Cast<byte, GfdEntry>(this.span[Unsafe.SizeOf<GfdHeader>()..]); // original: sizeof(GfdHeader)
    ...
    public struct GfdHeader
    {
        ...
        // public fixed byte Signature[8]; // original: requires unsafe
        ...
        // public fixed byte Padding[4]; // original: requires unsafe
        public uint Padding;
    }
